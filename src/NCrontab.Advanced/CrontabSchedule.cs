...
namespace NCrontab.Advanced
{
	public class CrontabSchedule
	{
		...
		private DateTime InternalGetNextOccurence(DateTime baseValue, DateTime endValue)
		{
			...
			if (isYearFormat)
				yearFilters = Filters[CrontabFieldKind.Year].Where(x => x is ITimeFilter).Cast<ITimeFilter>().ToList();
		...
		{
			return Filters.All(fieldKind => fieldKind.Value.Any(filter => filter.IsMatch(value)));
		}
		...
		public static void CheckForIllegalFilters(Dictionary<CrontabFieldKind, List<ICronFilter>> filters)
		{
			...
			if (monthSingle.Any()
			    ...
			    daySingle.All(x => x.SpecificValue == 30 || x.SpecificValue == 31))
				throw new CrontabException("Nice try, but February 30 and 31 don't exist.");
		}

		private static List<SpecificFilter> GetSpecificFilters(Dictionary<CrontabFieldKind, List<ICronFilter>> filters,
